...
import { useManutencoes, Manutencao } from '@/lib/contexts/ManutencaoContext';
import { useEquipamentos } from '@/lib/contexts/EquipamentoContext';
import { useAlertas } from '@/lib/contexts/AlertaContext';
import { Save, X, Calendar } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';

export default function ManutencaoFormContainer({ manutencaoId = null, equipamentoId = null }) {
  const router = useRouter();
  const { getManutencao, createManutencao, updateManutencao, loading, error } = useManutencoes();
  const { getEquipamento } = useEquipamentos();
  const { createAlerta } = useAlertas();
  
  // Estado para o equipamento
  const [equipamento, setEquipamento] = useState(null);
  
  // Estado para o formulário
  const [formData, setFormData] = useState<Manutencao>({
    equipamento_id: Number(equipamentoId) || 0,
    tipo: 'Preventiva',
  ...
  useEffect(() => {
    const fetchManutencao = async () => {
      if (manutencaoId) {
        const manutencao = await getManutencao(Number(manutencaoId));
        if (manutencao) {
          setFormData(manutencao);
          if (manutencao.equipamento_id) {
            const equip = await getEquipamento(manutencao.equipamento_id);
            if (equip) {
        ...
        setFormData(prev => ({ ...prev, equipamento_id: Number(equipamentoId) }));
    ...
    fetchManutencao();
  }, [manutencaoId, equipamentoId, getManutencao, getEquipamento]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (manutencaoId) {
        ...
        const success = await updateManutencao(Number(manutencaoId), formData);
        
        // Se houver data para próxima manutenção, criar alerta
        if (success && formData.data_proxima_manutencao) {
          await createAlerta({
            equipamento_id: formData.equipamento_id,
            data_alerta: formData.data_proxima_manutencao,
            tipo_manutencao: formData.tipo,
            descricao: `Manutenção ${formData.tipo.toLowerCase()} programada`,
            status: 'Pendente'
          });
        }
        
        if (success) {
          router.push(`/equipamentos/${formData.equipamento_id}`);
        }
      } else {
        // Criar nova manutenção
        const newId = await createManutencao(formData);
        
        // Se houver data para próxima manutenção, criar alerta
        if (newId && formData.data_proxima_manutencao) {
          await createAlerta({
            equipamento_id: formData.equipamento_id,
            data_alerta: formData.data_proxima_manutencao,
            tipo_manutencao: formData.tipo,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">
          {manutencaoId ? 'Editar Registro de Manutenção' : 'Novo Registro de Manutenção'}
        </h1>
        <div className="flex space-x-2">
          <Link
            href={equipamento ? `/equipamentos/${equipamento.id}` : '/equipamentos'}
